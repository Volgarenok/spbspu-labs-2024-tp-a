...
{
  if (part.empty()) {
    std::cerr << "Error: key-value part in the input." << std::endl;
    return false;
  }
...
{
  constexpr auto NoPos = std::strng::npos;
  ...
  if (line.empty()) {
    ...
    for (size_t i = 0; i < KeyCount; ++i) {
      ...
      if (to == NoPos || !parse(std::string(line.c_str() + from), dataStruct)) {
        std::cerr << "Error: Failed to perse key-value pair in the input: " << line << std::endl;
        stream.setstate(FailBit);
        return stream;
      }
      from = to;
