...
void kazennov::add_word(std::istream & in, std::map< std::string, FrequencyDict >&mapOfDicts)
{
  ...
  in >> word >> dictionary;
  auto m = mapOfDicts.find(dictionary);
  if (word == "" || dictionary == "")
  {
  ...
  {
    if (m->second.dict_.find(word) == mapOfDicts.find(dictionary)->second.dict_.end())
    {
      m->second.dict_.insert(std::pair<std::string, unsigned int>(word, 1));
    }
    else
    {
      m->second.dict_.find(word)->second++;
    }
  }
}
...
void kazennov::get_frequency(std::istream& in, std::ostream& out,
...
void kazennov::delete_word(std::istream& in, std::map<std::string, FrequencyDict>& mapOfDicts)
{
  ...
  {
    if (m->second.dict_.find(word) == m->second.dict_.end())
    {
      throw std::invalid_argument("no word in dictionary");
    }
    else
...
void kazennov::get_word_with_highest_frequency(std::istream& in, std::ostream& out, std::map<std::string, FrequencyDict>& mapOfDicts)
{
  ...
  compare = log;
  std::transform(strInput.begin(), strInput.end(), std::back_inserter(searchingDicts),
    [&](std::string name) { if (mapOfDicts.find(name) == mapOfDicts.end())
  {
...
void kazennov::get_count_of_word(std::istream& in, std::ostream& out, std::map<std::string, FrequencyDict>& mapOfDicts)
{
  ...
  std::transform(searchingDicts.begin(), searchingDicts.end(), std::back_inserter(Frequencies),
    [](std::pair<std::string, unsigned int> t) {return t.second; });
