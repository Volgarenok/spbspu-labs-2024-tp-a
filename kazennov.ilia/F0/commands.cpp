  ...
  if (mapOfDicts.find(dictionary) == mapOfDicts.end())
  {
	...
	if (f.is_open())
	{
	  ...
	  while (std::getline(f, line))
	  {
		...
		std::for_each(line.begin(), line.end(), [&](char t) {if ((int)t != 32) word += t; else { add_wordForFile(word, temp); word = ""; }});
	  }
	}
	mapOfDicts.insert(std::pair< std::string, FrequencyDict >(dictionary, temp));
  }
  else
  {
	...
	if (f.is_open())
	{
	  ...
	  while (std::getline(f, line))
	  {
		...
		std::for_each(line.begin(), line.end(), [&](char t) {if ((int)t != 32 && word.size() != 0) word.push_back(t); else { add_wordForFile(word, mapOfDicts.find(dictionary)->second); word = ""; }});
	  }
	}
  }
}
